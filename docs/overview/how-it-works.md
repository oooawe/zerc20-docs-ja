...
* そのバーンアドレスに指定のトークンを送信していること（バーンアドレスに到達していること）
...
{% hint style="info" %}
フロントエンド（ウェブアプリ）は現在、送信者がバーンアドレスを生成する方式のみ対応しています。CLIは両方の方式をサポートしています。
{% endhint %}


### 4. 金額のプライバシーについて

転送・Withdrawal の金額は **オンチェーン上で公開されます** 。`123.456789 zUSDC` のような特殊な金額は、トランザクションの紐付けに利用される可能性があります（Amount Fingerprinting）。
...
技術的な詳細は [アーキテクチャー概要](../developers/architecture.md) を参照してください。
