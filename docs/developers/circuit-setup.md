...
* **Indexer の実行**：Indexer が転送 Root 遷移の Nova Proof を生成する際に使う
* **Decider Prover の実行**：Decider Prover がオンチェーン検証用に Nova Proof を Decider Proof に最終化する際に使う
* **CLI での Proof 生成**：CLI が引き出し Proof（Nova・Groth16 両方）を生成する際に使う
